...
def load_data():
    try:
        ...
        print(f"⚠️ Database yuklanmadi: {e}")
...
def save_data(data):
    try:
        ...
        print(f"⚠️ Database saqlanmadi: {e}")
...
async def check_subscription(client, user_id):
    ...
    for ch in data["channels"]:
        try:
            ...
            print(f"⚠️ Kanal tekshiruvda xato {ch}: {e}")
            continue
...
async def start_handler(client, message: Message):
    try:
        ...
        print(f"❌ Start handler xato: {e}")
        await message.reply_text("⚠️ Xatolik yuz berdi. Iltimos qaytadan urinib ko'ring.")
...
async def message_handler(client, message: Message):
    try:
        ...
        print(f"❌ Message handler xato: {e}")
        if is_admin(user_id):
            await message.reply_text("⚠️ Xatolik yuz berdi. Admin panel:", reply_markup=admin_panel_keyboard())
        else:
            await message.reply_text("⚠️ Xatolik yuz berdi. Iltimos qaytadan urinib ko'ring.")
...
async def media_handler(client, message: Message):
    try:
        ...
        if action == "add_film" and temp_data[user_id].get("step") == "video":
            ...
            status_msg = await message.reply_text("📤 Reklama yuborilmoqda...", reply_markup=admin_panel_keyboard())
...
print(f"📊 Yuklangan ma'lumotlar:")
print(f"   👥 Foydalanuvchilar: {len(data['users'])}")
print(f"   🎬 Filmlar: {len(data['films'])}")
print(f"   👤 Adminlar: {len(data['admins'])}")
print(f"   📺 Kanallar: {len(data['channels'])}")
