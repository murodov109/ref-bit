...
async def text_handler(client, message: Message):
    ...
    if is_admin(user_id):
        if text == "➕ Film qo'shish":
            ...
            await message.reply_text("📢 Yuborish uchun xabar yuboring:\n\n(Matn, rasm yoki video yuborishingiz mumkin)")
            ...
            await message.reply_text(f"📺 Majburiy obuna kanallari:\n\n{channels_list}", reply_markup=keyboard)
            ...
            await message.reply_text(f"📨 Zayafka kanali:\n\n{request_text}", reply_markup=keyboard)
            ...
            await message.reply_text(f"🔗 URL linklar:\n\n{links_list}", reply_markup=keyboard)
    ...
    if code in data["films"]:
        ...
        await message.reply_video(film["file_id"], caption=caption, parse_mode=enums.ParseMode.MARKDOWN)
    else:
...
async def video_handler(client, message: Message):
    user_id = message.from_user.id
    if not is_admin(user_id) or user_id not in temp_data:
        return
...
async def photo_handler(client, message: Message):
    ...
    if temp_data[user_id]["action"] == "broadcast":
        success = 0
        failed = 0
        status_msg = await message.reply_text("📤 Yuborilmoqda...")
